...
unique_maindecaychannel = list(set(df['H5p_maindecaychannel'])) 
...
sc = ax.scatter(
        ...
        c=df['H5p_maindecaychannel'].map(maindecaychannel_colors),  # third variable represented as a color gradient
...
ax.legend(handles=handles_list, loc='lower left', ncol=2, fontsize='small')
...
# TITLE OF PLOT
ax.set_title(r'MAIN DECAY CHANNELS: $H^{+}_5$' + '\n' + 'GM MODEL + TYPE-II SEESAW\n' + '(RANDOM SCAN; N=100.000)')

# SAVE PLOT
plt.savefig('M3M5_vDELTA_H5pMDCH_GMSEESAW.png',
        bbox_inches='tight',
        dpi=100)
