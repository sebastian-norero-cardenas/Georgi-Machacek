...
INFO: Last update -> December 20, 2022.
"""
...
unique_maindecaychannel = list(set(df['H5z_maindecaychannel'])) 
...
sc = ax.scatter(
        ...
        y=np.abs(df['m3 [GeV]'] - df['m5 [GeV]']),  # second variable scattered over the y-axis
        c=df['H5z_maindecaychannel'].map(maindecaychannel_colors),  # third variable represented as a color gradient
...
ax.legend(handles=handles_list, loc='lower left', ncol=3, fontsize='small')

# AXES LABELS
ax.set_xlabel(r'$v_\Delta$  [GeV]')
ax.set_ylabel(r'$|m_3 - m_5|$  [GeV]')
...
# TITLE OF PLOT
ax.set_title(r'MAIN DECAY CHANNELS: $H^{0}_5$' + '\n' + 'GM MODEL + TYPE-II SEESAW\n' + '(RANDOM SCAN; N=100.000)')

# SAVE PLOT
plt.savefig('M3M5_vDELTA_H5zMDCH_GMSEESAW.png',
        bbox_inches='tight',
        dpi=100)
