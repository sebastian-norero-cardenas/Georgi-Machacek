...
unique_maindecaychannel = list(set(df['H3z_maindecaychannel'])) 
...
sc = ax.scatter(
        ...
        c=df['H3z_maindecaychannel'].map(maindecaychannel_colors),  # third variable represented as a color gradient
...
ax.set_title(r'MAIN DECAY CHANNELS: $H^{0}_3$' + '\n' + 'GM MODEL + TYPE-II SEESAW\n' + '(RANDOM SCAN; N=100.000)')

# SAVE PLOT
plt.savefig('M3M5_vDELTA_H3zMDCH_GMSEESAW.png',
        bbox_inches='tight',
        dpi=100)
